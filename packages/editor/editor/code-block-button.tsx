    ...
    return (
      <Button
        ...
        className={`${className.trim()} ${
          isActive
            ? 'bg-important hover:bg-important-hover active:bg-important-active text-background dark:text-foreground'
            : ''
        } `}
        disabled={isDisabled}
        data-style='ghost'
        ...
        {children || (
          <>
            <Icon name='Code' className={isActive ? 'text-background dark:text-foreground' : ''} />
            {text && <span>{text}</span>}
          </>
        )}
      </Button>

  ...
  ({ children, className = '', variant = 'fixed', ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={`flex items-center gap-1 p-2 bg-background border-border overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] min-h-[48px] ${className}`}
        {...props}>
        {children}
      </div>
    )
...
export const ToolbarGroup = React.forwardRef<HTMLDivElement, BaseProps>(
  ({ children, className = '', ...props }, ref) => {
    ...
    return (
      <div ref={mergeRefs([groupRef, ref])} role='group' className={`flex items-center gap-1 ${className}`} {...props}>
        {children}
      </div>
    )
  }
